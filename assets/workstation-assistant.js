(function ($) {
  ...
  var checks = {
    ln: {
      ...
      steps: [
        ...
        ['复核网络策略', '检查防火墙、ACL 与跨网段路由后重新执行 Checklist']
      ],
      assistantContext: 'PackingTable · Infor LN 连通性失败'
    },
    pcs: {
    ...
    sql: {
      state: 'passed', kicker: '运行正常', title: 'SQL Server 连通性正常', badge: '通过',
      ...
      priority: '无需处理',
      steps: [
        ['保持监控', '按 Checklist 周期执行端口与查询健康检查'],
        ['关注错误率', '若业务 API 出现数据库异常，再联动查看 SQL Server'],
        ['无需变更', '当前不需要人工干预']
      ],
      assistantContext: 'PackingTable · SQL Server 连通性正常'
    },
    agent: {
      ...
      explain: 'Agent 已成功加载当前 Checklist v3，并可与 WPF 助手进行本地状态通信。',
  ...
  var localeOverrides = {
    ...
    'th-TH': {
      ...
      sql:{kicker:'ปกติ',title:'การเชื่อมต่อ SQL Server ปกติ',badge:'ผ่าน',sub:'เวิร์กสเตชันเข้าถึงพอร์ต PESDB ได้',reason:'เครือข่ายฐานข้อมูลปกติ',explain:'TCP 1433 เข้าถึงได้',impact:'การค้นข้อมูลและ log ปกติ',scope:'ไม่พบปัญหาเครือข่ายฐานข้อมูล',priority:'ไม่ต้องดำเนินการ'},
      agent:{kicker:'ปกติ',title:'สภาพแวดล้อม Agent ปกติ',badge:'ผ่าน',sub:'cache, Named Pipe และพื้นที่ดิสก์พร้อมใช้งาน',reason:'ไม่พบปัญหา Agent ในเครื่อง',explain:'Agent โหลด Checklist v3 และสื่อสารกับ WPF ได้',impact:'การตรวจและรายงานสถานะทำงานปกติ',scope:'ไม่ต้องรีสตาร์ต',priority:'ไม่ต้องดำเนินการ'}
    }
  };

  var navLocales = {
    'zh-CN': {ln:['Infor LN 连通性','失败'],pcs:['PCS 服务连通性','通过'],sql:['SQL Server 连通性','通过'],agent:['Agent 运行环境','通过']},
    'en-US': {ln:['Infor LN Connectivity','Failed'],pcs:['PCS Connectivity','Passed'],sql:['SQL Server Connectivity','Passed'],agent:['Agent Runtime','Passed']},
    'es-MX': {ln:['Conectividad Infor LN','Falló'],pcs:['Conectividad PCS','Correcto'],sql:['Conectividad SQL Server','Correcto'],agent:['Entorno de Agent','Correcto']},
    'th-TH': {ln:['การเชื่อมต่อ Infor LN','ล้มเหลว'],pcs:['การเชื่อมต่อ PCS','ผ่าน'],sql:['การเชื่อมต่อ SQL Server','ผ่าน'],agent:['สภาพแวดล้อม Agent','ผ่าน']}
  };
  ...
  var replies = {
    ...
    '这个异常影响什么业务': '该异常属于 <strong>Infor LN 连通性</strong>。持续失败时可能影响需要访问 LN 的报工、订单校验或报表查询。PCS 和 SQL Server 当前正常，因此故障范围暂时集中在 LN 链路。',
    '为什么 LN 连不上？': '从当前 Checklist 证据看，TCP 连接在建立阶段失败。最常见原因是 <strong>目标服务未监听、端口被防火墙拦截或服务器不可达</strong>。建议先进行端口连通测试，再决定是否需要服务端处理。'
  };
  ...
  function ask(text) {
    ...
    setTimeout(function () {
      ...
      if (!answer) {
        ...
        else answer = '当前选中的 Checklist 项状态正常。正式接入 Dify 后，我可以继续结合历史趋势、知识库 SOP、日志和中央 API 状态进行分析。';
      }
      appendBot(answer);
    }, 560);
  }
